...
contract POSTimeToken is MintableToken {	
    
  string public constant name = "POST";
   
  string public constant symbol = "POS Time token";
...
contract Crowdsale is StagedCrowdsale {
  ...
  uint16 public foundersTokensPercent;
  
  uint public price;

  MintableToken public token = new POSTimeToken();
  ...
  function setFoundersTokensPercent(address newFoundersTokensPercent) onlyOwner {
    foundersTokensPercent = newFoundersTokensPercent;
  }

  function setPrice(address newPrice) onlyOwner {
    price = newPrice;
  }

  function createTokens() whenNotPaused saleIsOn payable {
    require(msg.value > 0);
    ...
    uint foundersTokens = tokens.div(100).mul(foundersTokensPercent);
    token.mint(this, tokensWithBonus.add(foundersTokens));
